...
public class Systems {
  private final Map<Class<? extends System>, Function<SystemContext, System>> factories =
      new HashMap<>();
  ...
  // The default is that it is initialized when we start, but some plugins may choose to initialize
  // later
  ...
  public <T extends System> Optional<T> get(Class<T> clazz) {
    ...
    if (instances.get(clazz) == null) {
      //            java.lang.System.err.println("System " + clazz.getName() + " is not initialized
      // yet.");
      return Optional.empty();
    }
      ...
      try {
        if (instance == null) {
          java.lang.System.out.println(
              "RuntimeSystems: Plugin factory returned null for "
                  + clazz.getName()
        ...
        if (instance.getSystemFlag(SystemFlag.IS_AUTO_START_DISABLED)) {
          java.lang.System.out.println(
              "RuntimeSystems: Plugin "
                  + clazz.getName()
                  + " is not set to auto-start, skipping initialization.");
          continue;
        }
        instance.activate(arguments);
        instance.setSystemState(
            SystemState.ACTIVE); // set the state to active after successful activation
        instances.put(
            clazz, instance); // we only care about the instance if it initialized successfully
      } catch (Exception e) {
  ...
  public <T extends System> void loadSystem(
      Class<T> clazz, RuntimeArguments arguments, SystemContext context) throws Exception {
