...
import client.core.gui.manager.GuiHoverManager;
...
public class GuiScrollPanel extends GuiContainer {
  ...
  protected void onRender(Renderer renderer) {
    ...
    //        scrollTransform.translate((int) -scrollState.getScrollXOffset(), (int)
    // -scrollState.getScrollYOffset());
    var scrollTransform =
        Transform.createTranslate(
            (int) -scrollState.getScrollXOffset(), (int) -scrollState.getScrollYOffset());
  ...
  private void renderHorizontalScrollbar(Renderer renderer) {
    ...
    if (thumbBounds != null) {
      renderer.setColor(
          isDraggingHorizontal ? new Color(80, 80, 80, 200) : new Color(120, 120, 120, 180));
      ...
      renderer.drawLine(
          centerX - 3, thumbBounds.y + 2, centerX - 3, thumbBounds.y + 2 + lineHeight);
  ...
  private void renderVerticalScrollbar(Renderer renderer) {
    ...
    if (thumbBounds != null) {
      renderer.setColor(
          isDraggingVertical ? new Color(80, 80, 80, 200) : new Color(120, 120, 120, 180));
      renderer.fillRect(thumbBounds.x, thumbBounds.y, thumbBounds.width, thumbBounds.height);
  ...
  public GuiEventState processMouseEvent(MouseEvent e) {
    ...
    if (!isHovered && !GuiMouseCaptureManager.getInstance().isCapturedComponent(this))
      return GuiEventState.NOT_CONSUMED;

    // Check to see if there is a scroll bar interaction (click, release or drag).  If there isn't,
    // but we are hovered
    // return true to ensure we consume the event.
    if (isPointOnScrollbar(localPoint)
  ...
  private boolean handleScrollbarInteraction(MouseEvent e, Point localPoint) {
    ...
    if (isDraggingVertical || isDraggingHorizontal) {
      if (e instanceof MouseEvent.Dragged) {
        ...
        Point dragDelta = new Point(localPoint.x - dragStart.x, localPoint.y - dragStart.y);
        ...
        if (isDraggingVertical) {
          ...
          scrollState.scrollTo(dragStartScrollX, dragStartScrollY + (dragDelta.y * scrollFactor));
          ...
          scrollState.scrollTo(dragStartScrollX + (dragDelta.x * scrollFactor), dragStartScrollY);
        }
        return true;
      }
  ...
  public static class GuiScrollState {
    ...
    @Getter private int contentWidth = 0;
    @Getter private int contentHeight = 0;
