...
public class GuiSlider extends GuiComponent {
  ...
  @Getter @Setter private double minValue = 0;
  @Getter @Setter private double maxValue = 100;
  @Getter @Setter private double value = 50;
  @Getter @Setter private boolean vertical = false;
  // Styling properties
  @Getter @Setter private Color trackColor = new Color(200, 200, 200);
  @Getter @Setter private Color thumbColor = new Color(100, 100, 150);
  @Getter @Setter private Color thumbHoverColor = new Color(120, 120, 180);
  @Getter @Setter private Color thumbActiveColor = new Color(80, 80, 130);
  // Thumb dimensions
  @Getter @Setter private int thumbWidth = 16;
  @Getter @Setter private int thumbHeight = 24;
  // State tracking for drag operations
  private boolean isDragging = false;
  private Point dragLastLocation = null;
  // Value change callback
  @Setter private Consumer<Double> onValueChanged = null;
  ...
  private int valueToPosition() {
    ...
    return vertical ? (int) (valuePercent * height) : (int) (valuePercent * width);
  }

  private double positionToValue(int position) {
    // Converts the given position to a slider value
    double percent = vertical ? 1.0 - (double) position / height : (double) position / width;
  ...
  private boolean isPointOnThumb(Point point) {
    if (vertical) {
      ...
      return point.x >= thumbX
          && point.x <= thumbX + thumbWidth
          && point.y >= thumbY - thumbHeight / 2
          && point.y <= thumbY + thumbHeight / 2;
    } else {
      int thumbX = valueToPosition();
      int thumbY = (height - thumbHeight) / 2;

      return point.x >= thumbX - thumbWidth / 2
          && point.x <= thumbX + thumbWidth / 2
          && point.y >= thumbY
          && point.y <= thumbY + thumbHeight;
    }
  }
  ...
  public GuiEventState processMouseEvent(MouseEvent e) {
    ...
    switch (e) {
      ...
      case MouseEvent.Dragged _ when isDragging
          && GuiMouseCaptureManager.getInstance().isCapturedComponent(this) -> {
        updateDrag(point);
        return GuiEventState.CONSUMED;
      }

        // End drag operation
      case MouseEvent.Released _ when isDragging
          && GuiMouseCaptureManager.getInstance().isCapturedComponent(this) -> {
        stopDrag();
        return GuiEventState.CONSUMED;
      }
  ...
  private void updateDrag(Point point) {
    double delta =
        dragLastLocation != null
            ? (vertical ? point.y - dragLastLocation.y : point.x - dragLastLocation.x)
            : 0;
