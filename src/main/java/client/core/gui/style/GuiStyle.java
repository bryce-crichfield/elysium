...
public class GuiStyle {
  @Getter private final GuiBackground background;
  @Getter private final GuiBorder border;
  @Getter private final GuiFont font;
  ...
  public static GuiStyle deserialize(com.google.gson.JsonObject json) {
    var background =
        GuiBackground.deserialize(json.getAsJsonObject("background"))
            .orElse(new GuiBackground.Fill(Color.BLACK));
    var border =
        GuiBorder.deserialize(json.getAsJsonObject("border")).orElse(new GuiBorder(Color.BLACK, 0));
    var font =
        GuiFont.deserialize(json.getAsJsonObject("font"))
            .orElse(new GuiFont(Color.WHITE, "/fonts/arial", 12));

    var hoverStyle = Optional.ofNullable(json.getAsJsonObject("hover")).map(GuiStyle::deserialize);
    var focusStyle = Optional.ofNullable(json.getAsJsonObject("focus")).map(GuiStyle::deserialize);
    var disabledStyle =
        Optional.ofNullable(json.getAsJsonObject("disabled")).map(GuiStyle::deserialize);
