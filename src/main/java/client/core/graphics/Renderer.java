...
import client.core.graphics.font.FontRenderer;
import client.core.graphics.paint.Paint;
import client.core.graphics.paint.SolidColorPaint;
import java.awt.*;
...
public class Renderer {
  ...
  @Setter @Getter private int lineWidth = 1;
  @Setter @Getter private Composite composite;
  @Getter private client.core.graphics.paint.Paint paint;
  ...
  public void pushClip(int x, int y, int width, int height) {
    ...
    if (clipStack.isEmpty()) {
      ...
      Rectangle untransformedIntersection =
          transformRectangle(transformedIntersection, inverseTransform);
  ...
  private void applyCurrentClip(Rectangle clip) {
    ...
    Vector2f clipBottomRight =
        transformStack.peek().apply(new Vector2f(clip.x + clip.width, clip.y + clip.height));
  ...
  public void drawRect(int x, int y, int width, int height) {
    ...
    // Temporarily push a new apply to the stack (fromScreenSpace) to take our screen space
    // coordinates
  ...
  public void fillRect(int x, int y, int width, int height) {
    ...
    if (paint instanceof client.core.graphics.paint.LinearGradientPaint gradient) {
  ...
  private void fillRectWithGradient(
      int x,
      int y,
      int width,
      int height,
      client.core.graphics.paint.LinearGradientPaint gradient) {
    pushTransform(Transform.fromScreenSpace(bufferWidth, bufferHeight));
    ...
    float gradientLength =
        (float) Math.sqrt(gradientDirX * gradientDirX + gradientDirY * gradientDirY);
  ...
  public void fillRoundRect(int x, int y, int width, int height, int radiusX, int radiusY) {
    ...
    drawCorner(
        transform, x + width - radiusX, y + radiusY, radiusX, radiusY, 270, 360); // top-right
    drawCorner(
        transform,
        x + width - radiusX,
        y + height - radiusY,
        ...
        90); // bottom-right
    drawCorner(
        transform, x + radiusX, y + height - radiusY, radiusX, radiusY, 90, 180); // bottom-left
  ...
  public void drawRoundRect(int x, int y, int width, int height, int radiusX, int radiusY) {
    ...
    glVertex2f(
        transform.apply(new Vector2f(x + radiusX, y)).x,
        transform.apply(new Vector2f(x + radiusX, y)).y);
    glVertex2f(
        transform.apply(new Vector2f(x + width - radiusX, y)).x,
        transform.apply(new Vector2f(x + width - radiusX, y)).y);
    drawArc(transform, x + width - radiusX, y + radiusY, radiusX, radiusY, 270, 360); // top-right
    glVertex2f(
        transform.apply(new Vector2f(x + width, y + radiusY)).x,
        transform.apply(new Vector2f(x + width, y + radiusY)).y);
    glVertex2f(
        transform.apply(new Vector2f(x + width, y + height - radiusY)).x,
        transform.apply(new Vector2f(x + width, y + height - radiusY)).y);
    drawArc(
        transform,
        x + width - radiusX,
        y + height - radiusY,
        ...
        90); // bottom-right
    glVertex2f(
        transform.apply(new Vector2f(x + width - radiusX, y + height)).x,
        transform.apply(new Vector2f(x + width - radiusX, y + height)).y);
    glVertex2f(
        transform.apply(new Vector2f(x + radiusX, y + height)).x,
        transform.apply(new Vector2f(x + radiusX, y + height)).y);
    drawArc(transform, x + radiusX, y + height - radiusY, radiusX, radiusY, 90, 180); // bottom-left
    glVertex2f(
        transform.apply(new Vector2f(x, y + height - radiusY)).x,
        transform.apply(new Vector2f(x, y + height - radiusY)).y);
    glVertex2f(
        transform.apply(new Vector2f(x, y + radiusY)).x,
        transform.apply(new Vector2f(x, y + radiusY)).y);
    glEnd();
  ...
  public void drawFrameBuffer(FrameBuffer buffer, int x, int y, int width, int height) {
    ...
    // Transform the coordinates - now using width and height parameters instead of fbWidth and
    // fbHeight
    ...
    // Draw the quad but inverted so it's flipped over the y-axis (I'm not sure why we need to do
    // this, but it works)
    glBegin(GL_QUADS);
    glTexCoord2f(0, 1);
    glVertex2f(topLeft.x, topLeft.y);
