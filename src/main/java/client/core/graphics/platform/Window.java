...
public class Window {
  ...
  public Window(int width, int height, Application game) {
    this.game = game;
    this.bufferWidth = Application.SCREEN_WIDTH;
    this.bufferHeight = Application.SCREEN_HEIGHT;
  ...
  private void setupInputCallbacks(Application game) {
    glfwSetKeyCallback(
        windowHandle,
    ...
    glfwSetCursorPosCallback(
        ...
        (window, xpos, ypos) -> {
          ...
          Optional<Point> point =
              transformWindowPointToViewport((int) xposArray[0], (int) yposArray[0]);
          if (point.isPresent()) {
  ...
  public void onRender(float updateTime, float renderTime) {
    ...
    glViewport(
        letterboxViewport.x,
        letterboxViewport.y,
        letterboxViewport.width,
        letterboxViewport.height);
