...
public class FontRenderer {
  ...
  public void setFont(String name, int size) {
    ...
    STBTruetype.stbtt_BakeFontBitmap(
        fontBuffer, size, bitmap, BITMAP_WIDTH, BITMAP_HEIGHT, FIRST_CHAR, charData);
  ...
  public FontInfo getFontInfo(String name, int size) {
    ...
    if (fontInfo == null) {
      throw new IllegalStateException(
          "Font " + fontKey + " is not cached, cannot load font " + fontKey);
    }
  ...
  public void drawString(String text, int x, int y, Color color) {
    ...
    for (int i = 0; i < text.length(); i++) {
      ...
      STBTruetype.stbtt_GetBakedQuad(
          charData, BITMAP_WIDTH, BITMAP_HEIGHT, c - FIRST_CHAR, xBuffer, yBuffer, quad, true);
