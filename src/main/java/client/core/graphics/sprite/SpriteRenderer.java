...
public class SpriteRenderer {
  ...
  private static final int VERTEX_SIZE =
      10 * Float.BYTES; // 3 position + 2 texCoord + 4 color + 1 texIndex
  ...
  private void setupBuffers() {
    ...
    vertexVertexBuffer.storeData(
        BufferUtils.createFloatBuffer(vertices.length), GL15.GL_DYNAMIC_DRAW);
  ...
  public void flush() {
    ...
    FloatBuffer vertexBuffer =
        BufferUtils.createFloatBuffer(
            spriteCount * VERTICES_PER_SPRITE * (VERTEX_SIZE / Float.BYTES));
    ...
    vao.bind();
    GL11.glDrawElements(
        GL11.GL_TRIANGLES, spriteCount * INDICES_PER_SPRITE, GL11.GL_UNSIGNED_INT, 0);
    vao.unbind();
