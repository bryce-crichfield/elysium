...
public class FadeTransition extends Transition {
  ...
  public FadeTransition(
      FrameBuffer sourceImage,
      FrameBuffer targetImage,
      Duration duration,
      Color color,
      Easing easing) {
    super(duration);
  ...
  public void render(Renderer graphics, int width, int height) {
    ...
    if (alpha < 0.5f) {
      ...
      color =
          new Color(color.getRed(), color.getGreen(), color.getBlue(), (int) (255 * (alpha * 2)));
    ...
    // Second phase (0.5 to 1.0): Fade in target image
      ...
      color =
          new Color(
              color.getRed(), color.getGreen(), color.getBlue(), (int) (255 * (2 - (alpha * 2))));
