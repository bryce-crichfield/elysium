...
public class Window {
    ...
    public Window(int width, int height, GameContext GameContext) {
        this.GameContext = GameContext;
        this.bufferWidth = GameContext.SCREEN_WIDTH;
        this.bufferHeight = GameContext.SCREEN_HEIGHT;
        ...
        // Create window
        windowHandle = glfwCreateWindow(width, height, "GameContext", NULL, NULL);
        if (windowHandle == NULL) {
            throw new RuntimeException("Failed to create the GLFW window");
        }

        // Setup mouse callbacks
        setupInputCallbacks(GameContext);
    ...
    private void setupInputCallbacks(GameContext GameContext) {
        glfwSetKeyCallback(windowHandle, (window, key, scancode, action, mods) -> {
            KeyEvent event = new KeyEvent(key, action);
            switch (action) {
                case GLFW_PRESS -> GameContext.getKeyboard().keyPressed(event);
                case GLFW_RELEASE -> GameContext.getKeyboard().keyReleased(event);
                default -> {
                }
            }
        });
        ...
        glfwSetMouseButtonCallback(windowHandle, (window, button, action, mods) -> {
            ...
            switch (action) {
                case GLFW_PRESS -> {
                    ...
                    GameContext.getMouse().mousePressed(event);
                    isDown[button] = true;
                }
                case GLFW_RELEASE -> {
                    ...
                    if (isDown[button]) {
                        ...
                        GameContext.getMouse().mouseClicked(new MouseEvent.Clicked(point.get(), button, 1, false));
                    }
                    GameContext.getMouse().mouseReleased(event);
                    isDown[button] = false;
                }
                default -> {
        ...
        glfwSetScrollCallback(windowHandle, (window, xoffset, yoffset) -> {
            ...
            if (point.isPresent()) {
                ...
                GameContext.getMouse().mouseWheelMoved(event);
            }
        });
    ...
    public void onRender(float updateTime, float renderTime) {
        ...
        // Create renderer and render the GameContext
