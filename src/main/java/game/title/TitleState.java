...
import core.gui.control.GuiSlider;
import core.gui.input.GuiHoverHandler;
...
import game.options.OptionsState;
...
public class TitleState extends GameState {
    ...
    public TitleState(GameContext gameContext) {
        super(gameContext);

        addBackground(Background.stars());

        // Main container
        var container = new GuiContainer(0, 0, GameContext.SCREEN_WIDTH, GameContext.SCREEN_HEIGHT);
        container.setLayout(new GuiNullLayout());

        // Calculate center position
        int centerX = GameContext.SCREEN_WIDTH / 2;
        int startY = GameContext.SCREEN_HEIGHT / 3;
        ...
        container.addChild(createMenuButton(centerX - BUTTON_WIDTH / 2, currentY, "New Game", () -> {
            ...
            gameContext.pushState(BattleState::new, transition);
        }));
        currentY += BUTTON_HEIGHT + BUTTON_SPACING;
        ...
        container.addChild(createMenuButton(centerX - BUTTON_WIDTH / 2, currentY, "Options", () -> {
            ...
            gameContext.pushState(OptionsState::new, Transitions.fade(Duration.ofMillis(300), Color.BLACK, Easing.easeIn()));
        }));
        currentY += BUTTON_HEIGHT + BUTTON_SPACING;
...
//        container.setBorder(new GuiBorder(Color.WHITE, 2));
    ...
    public GuiComponent createMenuButton(int x, int y, String text, Runnable onClick) {
        ...
        var mouseClick = GuiMouseHandler.onClick(() -> {
            onClick.run();
            gameContext.getAudio().play("future_ui/beep");
