...
public class ChatScene extends ApplicationScene {
  ...
  private boolean isNetworkingAvailable =
      false; // Default to true, will be set false if plugin fails to load
  ...
  public GuiComponent buildGui() {
    ...
    int screenWidth =
        Integer.parseInt(arguments.getOrDefault("screenWidth", "" + Application.SCREEN_WIDTH));
    int screenHeight =
        Integer.parseInt(arguments.getOrDefault("screenHeight", "" + Application.SCREEN_HEIGHT));
    ...
    sendButton.setPosition(
        (screenWidth - sendButton.getWidth()) / 2,
    ...
    inputField.setPosition(
        (screenWidth - inputField.getWidth()) / 2, screenHeight - 200); // Above the button
    inputField.setStyle(GuiStyle.builder().background(new GuiBackground.Fill(Color.RED)).build());
  ...
  public void onEnter() {
    ...
    // Ensure the NetworkingPlugin is loaded (is disabled by default, which allows for some scenes
    // to run without it)
    ApplicationRuntimeContext applicationRuntimeContext = getApplication().getRuntimeContext();
    if (!(applicationRuntimeContext.systems.getState(NetworkingSystem.class)
        == SystemState.ACTIVE)) {
      try {
        getApplication()
            .getRuntimeContext()
            .loadSystemBlocking(NetworkingSystem.class, getApplication());
